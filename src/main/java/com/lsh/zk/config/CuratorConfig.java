...
import com.lsh.zk.watch.ZookeeperWatches;
...
public class CuratorConfig {
    ...
    public CuratorFramework curatorClient() throws Exception {
        CuratorFramework client = CuratorFrameworkFactory.builder()
                //连接地址  集群用,隔开
                .connectString(curatorProperties.getIp())
                .connectionTimeoutMs(curatorProperties.getConnectionTimeoutMs())
                //会话超时时间
                .sessionTimeoutMs(curatorProperties.getSessionTimeOut())
                //设置重试机制
                .retryPolicy(new ExponentialBackoffRetry(curatorProperties.getSleepMsBetweenRetry(),curatorProperties.getMaxRetries()))
